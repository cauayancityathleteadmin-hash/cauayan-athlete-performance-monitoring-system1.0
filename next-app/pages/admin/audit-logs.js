      ...
      <AppShell session={session} isAdmin eyebrow="Activity trail" title="Audit Logs" active="/admin/audit-logs">
        ...
        <div className={styles.panel}>
          {loading ? <p className={styles.empty}>Loading audit log...</p> : error ? <p className={styles.formError}>{error}</p> : (
            <>
              <div className={styles.tableWrap}><table><thead><tr><th>When</th><th>Actor</th><th>Role</th><th>Action</th><th>Entity</th><th>Detail</th></tr></thead><tbody>{logs.map((log) => <tr key={log.id}><td data-label="When">{formatDate(log.createdAt)}</td><td data-label="Actor">{log.user?.email || "system"}</td><td data-label="Role">{log.user?.role || "—"}</td><td data-label="Action"><span className={`${styles.badge} ${styles.badgeActive}`}>{log.action}</span></td><td data-label="Entity">{log.entityType} {log.entityId ? `#${log.entityId}` : ""}</td><td data-label="Detail">{log.description || "—"}</td></tr>)}{!logs.length && <tr><td colSpan="6" className={styles.empty}>No log entries match the filters.</td></tr>}</tbody></table></div>
