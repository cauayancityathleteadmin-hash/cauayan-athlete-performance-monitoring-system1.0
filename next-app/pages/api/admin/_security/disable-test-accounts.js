...
export default async function handler(req, res) {
  ...
  const codes = ["COA-TEST01", "COA-TEST02", "COA-TEST03"];
  const results = [];
  try {
    const coaches = await prisma.coach.findMany({ where: { coachCode: { in: codes } }, include: { user: true } });
    ...
    for (const coach of coaches) {
      ...
      results.push(`${coach.coachCode}: user=${coach.user.username}/${coach.user.status} -> inactive`);
    }
