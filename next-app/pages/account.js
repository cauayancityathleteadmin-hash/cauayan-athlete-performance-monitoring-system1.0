...
export default function Account({ user, sports, session }) {
  ...
  const [csvMsg, setCsvMsg] = React.useState({ kind: "", text: "" });
  ...
  return (
      <ErrorBarrier>
        ...
        <main style={{ padding: 32, fontFamily: "sans-serif" }}>
          <p data-diag="render-probe">RENDER PROBE OK — role={user.role} sports={sports.length} coach={user.coach ? "yes" : "no"} tab={tab} sessionEmail={session?.user?.email}</p>
        </main>
      </ErrorBarrier>
    );
}