    ...
    <>
      ...
      <AppShell session={session} isAdmin={isAdmin} eyebrow="Evidence at a glance" title="Analytics" active="/analytics">
        ...
        <section className={styles.panel}>
          ...
          {insights.length ? <div className={styles.tableWrap}><table><thead><tr><th scope="col">Athlete</th><th scope="col">Metric</th><th scope="col">Latest</th><th scope="col">Percentile</th><th scope="col">Trend vs prior</th></tr></thead><tbody>{insights.map((row, i) => <tr key={i}><td><strong>{row.athleteName}</strong></td><td>{row.metricName}<small>{row.unit}</small></td><td><strong>{row.value}</strong>{row.unit ? <small>{row.unit}</small> : null}</td><td>{row.band >= 75 ? <strong>{row.band}%</strong> : row.band >= 25 ? <span>{row.band}%</span> : <span className={styles.mutedSmall}>{row.band}%</span>}</td><td>{trendCell(row.trend)}</td></tr>)}</tbody></table></div> : <p className={styles.empty}>No numeric results with trends yet.</p>}
        </section>
