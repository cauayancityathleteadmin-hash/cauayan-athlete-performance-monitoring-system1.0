...
## Import template, official reports & data-model sync (standing convention)
- Athlete bulk import: `lib/athlete-import-headers.js` is the single source of truth for the sample template columns. The parser (`lib/athlete-import.js`) and the "Download template (CSV)" button (`pages/athletes.js`) both read from it. NEVER re-declare the header list inline anywhere else, and never update one copy without the other.
- Checklist item (required with every athlete data-model change — added field, renamed field, changed format): in the SAME change, update `ATHLETE_IMPORT_HEADERS` + `ATHLETE_IMPORT_SAMPLE` in `lib/athlete-import-headers.js`, then re-run the import round-trip test (success with the new template + a deliberately bad file) before considering the change done.
- Official printable reports (`/reports`): keep them formal and accurate to the data model. Formatting lives in `styles/report-print.css` (Times New Roman, A4 portrait 12/14mm margins, print-only chrome hiding). Report content lives in `pages/reports.js` (AthleteReportCard / CoachReportCard / PerformanceSummary). When data fields change, check the matching report section is updated in the same change; verify signature/certification blocks, section numbering, and pagination behavior still hold.
- Formal report signature chain (keep consistent): "Prepared by" (authorized user) → "Reviewed by" (division head) → "Certified Correct" (City Sports Development Officer).
